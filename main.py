...
import os
def cprint(szoveg=""):
    print(szoveg.center(os.get_terminal_size().columns))
import time
import sys
import json
...
def boot_animation():
    ...
    cprint("")
    cprint("KJI MYRA 1.0 INTERNAL SYSTEM")
    cprint("SYSTEM INITIALIZATION")
    cprint("")

    loading_steps = [
        "[ OK ] VEHICLE DATABASE CONNECTED",
        "[ OK ] USER AUTHENTICATION ONLINE",
        "[ OK ] LOGGING SERVICE ACTIVE",
        "[ OK ] INTERNAL TERMINAL SECURED",
        "[ OK ] ENCRYPTED SESSION ESTABLISHED",
    ]
    ...
    for i in range(101):
        bar = "█" * (i // 2)
        line = "[{:<50}] {}%".format(bar, i)

        sys.stdout.write("\r" + line.center(terminal_width))
        sys.stdout.flush()
        time.sleep(0.03)
...
def startup_menu():
    while True:
        ...
        cprint("[ 𝗡𝗨𝗠 𝟭 ]")
        print()
        cprint("BELÉPÉS A RENDSZERBE")
        cprint("[ 𝗡𝗨𝗠 𝟬 ]")
        print()
        cprint("PROGRAM BEZÁRÁSA")
        cprint("")
